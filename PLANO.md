...
## 13.3 Tela nova — "Talão de vias" (22/09/2026, ainda não publicada)

Depois de assinar um contrato real, o Paulo achou a tela "esquisita" no fluxo e no visual: "Baixar" não funcionava no chat, WhatsApp e e-mail não levavam o PDF, não dava para salvar numa pasta à escolha nem tirar o documento errado, e num contrato de ~60 páginas o "Página X de Y" não ajudava a achar o lugar. Contexto do produto em `PRODUCT.md`; direção escolhida por ele entre as opções da rodada de design: **Talão de vias** (bloco de recibo com carbono; seed `8e9368b5`).

- **Canhoto** (faixa amarela à esquerda; no chat, em cima): documento com Trocar/Remover, **Onde assinar** (todos os blocos do documento, com o nome de quem assina e a página), sua assinatura, histórico com carimbos ASSINADO/CANCELADO e o Nº da assinatura no registro. **Picote** separa do documento.
- **Achar o lugar**: `anchors.ts` passou a listar todos os blocos (CONTRATANTE, CONTRATADA, LOCATÁRIA, Testemunha 1…, "Pela CONTRATANTE", nome acima ou abaixo do rótulo), sem pegar frase do corpo; a tela abre direto no bloco escolhido. Documento inteiro numa coluna que rola (só as páginas perto da vista são desenhadas), campo de página, régua de páginas com os lugares marcados, zoom e busca de texto. Arrastar a assinatura leva a outra página; "Outro lugar" + clique para PDF escaneado.
- **Entrega pelo computador** (`src/core/deliver.ts`, sem shell, argumentos fixos): abrir, mostrar na pasta, **salvar uma cópia em…** (janela do sistema; a tela acompanha a tarefa, porque o app de chat corta chamadas longas), **Apple Mail/Outlook com o PDF anexado** (`open -a`, sem pedir permissão ao macOS), **Gmail/WhatsApp guiados** (PDF copiado como arquivo — o mesmo que o Finder põe na área de transferência — e a pasta aberta ao lado; quem envia é a pessoa), copiar o PDF. Só aparece o que existe no computador.
- **Entrada e saída**: Remover/Trocar não apagam o arquivo; **Desfazer** manda a cópia assinada para a Lixeira, registra `signature_undone` no log (nada é apagado dele) e reabre o pedido com o mesmo documento. O terminal espera a nova assinatura em vez de sair.
- **Novas tools só-da-tela** (MCP Apps): `remove_document`, `deliver`, `save_copy`, `save_job`, `undo`, `open_in_browser` (tela grande quando o chat não dá tela cheia; o link com o token vai direto ao navegador, não ao modelo). Rotas equivalentes no servidor HTTP.
- **Painel no chat**: tamanho P/M/G (fica guardado), zoom "Página inteira", e depois de assinar a aba "Ver o documento assinado". No compacto (chat ou janela estreita) o canhoto ocupa no máximo ~55% da altura (42% em painel baixo) e rola por dentro: o documento nunca some. "Salvar uma cópia em…" nunca substitui um arquivo que já existe (vira "-2").
- **Recursos novos** (escolhidos pelo Paulo, 22/09): **rubrica em todas as páginas** (desenhada uma vez, no canto de baixo à direita; arrastar numa página muda em todas; opcional também nas páginas com assinatura), **local e data** (cidade e data por extenso, nome e CPF opcionais; cidade/nome/CPF ficam nas preferências do cofre, só no computador; o registro guarda onde e quantas linhas, nunca o texto), **vários lugares** ("Assinar em mais um lugar" + clique na página) e **comprovante** (`src/core/receipt.ts`: PDF à parte, `<nome>.assinado.comprovante.pdf`, com data/hora, lugares, o desenho usado, SHA-256 do original e do assinado, como conferir e o que ele não é; o segundo clique abre o mesmo; desfazer manda os dois para a Lixeira). Tudo num salvamento incremental só. O desenho repetido (rubrica, vários lugares) entra uma vez no arquivo como Form XObject e cada página só aponta para ele: num contrato de 60 páginas, o acréscimo caiu de ~287 KB para ~94 KB (o resto é do PDF de teste). O agente pode pedir as opções já ligadas com `initials_all_pages` e `place_and_date` em `labsign_sign_document`; quem desenha e confere é a pessoa.
- **Verificado**: 81 testes (novos: lugares, remover/trocar, salvar cópia, entrega, desfazer, espera do terminal, rubrica/texto/vários lugares no mesmo salvamento, texto em pé em página girada, desenho compartilhado com recursos divididos entre páginas, comprovante); capturas em `.impeccable/review/`; revisão final independente. No Mac do Paulo: o script de copiar arquivo grava os mesmos tipos que o Finder e o de Lixeira funciona (um PDF de teste, `labsign-teste-lixeira.pdf`, ficou na Lixeira).
- **Instaladores de teste** `0.2.0-teste.1` a `teste.4`, montados fora do repositório e abertos no Claude Desktop do Paulo para ele testar no chat.
- **Falta testar de verdade** (precisa abrir janelas na frente da pessoa): `open -a Mail/Outlook` anexando; colar o PDF copiado no Gmail e no WhatsApp Web (Chrome e Safari); tela cheia no Claude Desktop; o painel no chat com o servidor novo (reiniciar o servidor MCP `labsign-dev` do projeto, ou instalar um `.mcpb` novo).
