...
- **Real preview:** the whole document in one scrolling column, with the signature on top. Drag it (even to another page), resize it (keyboard works too) or remove it. What you see is what ends up in the PDF.
- **Finds the spot, even in 60 pages:** lists every signature block in the document (Contratante, Contratada, Witness 1…, with the signer's name) and opens right at yours. One click moves to another. You can also type a page number, use the page ruler on the edge or search the text. If nothing is found (a scanned PDF), choose “Somewhere else” and click on the page.
- **Initials, place and date, more than one spot:** “Initial every page” puts your initials in the bottom corner of each page (draw them once; dragging them on one page moves them on all). “Write place and date” writes “Boston, September 22, 2026” next to the signature, with your name and ID number if you want (city, name and ID stay on your computer only). “Sign in one more place” adds the same signature somewhere else in the document with one click. Everything goes into the PDF in one save, and the initials are stored once in the file, so it barely grows.
- **Receipt:** a separate PDF, next to the signed copy, with the date and time, where it was signed, the signature used and the SHA-256 of the original and the signed file, plus how to check them. It says what it is and what it isn't: not a digital certificate, and on its own it doesn't identify the signer.
- **A way in and out at every step:** the stub on the left shows the document, where to sign, your signature and the history. You can replace or remove the wrong document (the file stays on your computer), cancel before signing and undo afterwards: the signed copy goes to the Trash and the document is ready to sign again.
...
- **Evidence record:** for each signature, it logs the date and time and the SHA-256 of the original and the signed file, in a hash-chained log. Undoing is logged too (nothing is deleted from it).
- **Send or keep, from your computer:** after signing, labsign itself opens the PDF, shows it in its folder, saves a copy wherever you choose (the system's save window) or copies the file so you can paste it anywhere. In **Apple Mail** and **Outlook**, a new message opens with the PDF already attached. In **Gmail** and **WhatsApp** (which don't take attachments through a link), the message or chat opens, the PDF is copied and its folder opens alongside: just paste (⌘V) or drag. You are always the one who presses Send.
